...
def read_patient_representation_and_label(data_source, info_folder, embed_folder, test_idx, data_fraction=1,
    ...
    if raw_data:
        ...
        test_pat_embed = np.load(os.path.join(
            embed_folder, '{}_pat_representation_{}_mine.npy'.format(data_source, test_idx)))
    test_label = np.load(os.path.join(info_folder, 'label_list_{}.npy'.format(test_idx)))
    test_disease = np.load(os.path.join(info_folder, 'disease_list_{}_mine.npy'.format(test_idx)))
    test_risk_factor = np.load(os.path.join(info_folder, 'risk_factor_list_{}_mine.npy'.format(test_idx)))
    test_category = np.load(os.path.join(info_folder, 'disease_category_list_{}_mine.npy'.format(test_idx)))
...
def main():
    ...
    print(test_pat_embed.shape, test_interact.shape)
